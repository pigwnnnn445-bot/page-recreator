...
const PromptGeneratorInner = ({
  ...
  <>
    ...
    <div className="p-4 flex flex-col gap-3 overflow-y-auto min-h-0 flex-1">
      ...
      {isOptimizing && (
        <div className="fixed inset-0 z-[9999] flex flex-col items-center justify-center gap-4 animate-fade-in bg-background/10 backdrop-blur-[1px]" style={{ height: '100dvh' }}>
    ...
    <>
      ...
      {isDesktop ? (
        ...
        <Drawer open={open} onOpenChange={(v) => !v && onClose()}>
          <DrawerContent className="max-h-[92dvh] bg-card border-border rounded-t-2xl p-0 flex flex-col transition-[height] duration-300" style={{ height: results ? '90dvh' : '55dvh' }}>
            <PromptGeneratorInner {...innerProps} />
          </DrawerContent>
        </Drawer>
      )}
      ...
      {isDesktop ? (
        ...
        <Drawer open={editDialogOpen} onOpenChange={setEditDialogOpen}>
          <DrawerContent className="max-h-[90vh] bg-card border-border rounded-t-2xl p-0 flex flex-col">
